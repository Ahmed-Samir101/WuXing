...
bool playerCollisionBending(SDL_Rect dstattack, int dir) {
    if (dir == 1) {
...
int bendingSkill(int& bending, SDL_Texture* texture) {
    ...
    if (player.playerAttack == true) {
        if (playerCollisionBending(dstRect, initialDir)) {
            // printf("collision\n");
            playerAttack();
            xpValue += playerDamage;
...
int blockingSkill(int& bending, SDL_Texture* texture) {
    static Uint32 blockingStartTime = SDL_GetTicks();
    Uint32 currentTime = SDL_GetTicks();
    Uint32 duration = 1000; // Duration in milliseconds (adjust as needed)

    if (player.isBlocking) {
        int totalY = player.y + 64;
        SDL_Rect dstRect = { player.x + 20, totalY, 140, 140 };

        SDL_RenderCopyEx(renderer, texture, nullptr, &dstRect, 0, nullptr, (playerDir == 2) ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE);
    }

    if (currentTime - blockingStartTime >= duration) {
        player.isBlocking = false;
        bending = 0;
        blockingStartTime = SDL_GetTicks(); // Reset the starting time when blocking is finished
    }
...
void setPlayerBendProperties(SDL_Texture& texture, int frame_num, int frame_Y, int frame_width, int frame_height, int invDir, int v1, int v2) {
    if(playerbend.bending == 0 && startcooldown == 0){
        playerbend.bending = 1;
    }
    if(playerbend.bending == 0 && startcooldown == 1){
        playerbend.bending = 0; 
    }
...
void enemyBounce() {
    enemy.dx = 4;
    enemy.dx = ((playerDir == 1) ? -6 : 6) * enemy.dx; // Reverse the player's velocity
    enemy.x += enemy.dx;  // Update the player's position using the reversed velocity
}
